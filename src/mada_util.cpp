...
void mada_util::getPlannerParams(ros::NodeHandle nh, string &robot_name, string &map_topic, string &pose_topic, string &robot_frame, string &laser_topic, string &pathToFollow_topic, double &sX, double &sY, double &laser_range, double &maxLinearVel, double &maxAngularVel, double &occup_thres, int &forget_dynamic_areas, float &max_mission_time, double &inflFact, int &monitoring_type)
{
    ...
    nh.getParam("pathToFollow_topic", pathToFollow_topic);
    nh.getParam("sX", sX);
    nh.getParam("sY", sY);
    nh.getParam("laser_range", laser_range);
    nh.getParam("maxLinearVel", maxLinearVel);
    nh.getParam("maxAngularVel", maxAngularVel);
    nh.getParam("occup_thres", occup_thres);
    nh.getParam("forget_dynamic_areas", forget_dynamic_areas);
    nh.getParam("max_mission_time", max_mission_time);
    nh.getParam("inflFact", inflFact);
    nh.getParam("monitoring_type", monitoring_type);
}

void mada_util::getObstaclesParams(ros::NodeHandle nh, string &map_topic, string &footprint_topic, int &Nobst, double &sX, double &sY, double &maxLinearVel, double &maxAngularVel, double &linearAcc, double &angularAcc, int &seed, int &navAreaType)
{
    nh.getParam("map_topic", map_topic);
    nh.getParam("footprint_topic", footprint_topic);
    nh.getParam("Nobst", Nobst);
    nh.getParam("sX", sX);
    nh.getParam("sY", sY);
    nh.getParam("maxLinearVel", maxLinearVel);
    nh.getParam("maxAngularVel", maxAngularVel);
    nh.getParam("linearAcc", linearAcc);
    nh.getParam("angularAcc", angularAcc);
    nh.getParam("seed", seed);
    nh.getParam("navAreaType", navAreaType);
}
...
void mada_util::setFollowerPathPoses(vector<geometry_msgs::Pose> &pathPoses)
{
    for(int i=0; i<pathPoses.size(); i++)
        pathPoses[i].orientation.x = 0.00001;
}
...
{
    cout<<"("<<cmd_vel.linear.x<<", "<< cmd_vel.angular.z<<")";
}
