...
int main(int argc, char** argv) {
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        */
        ...
        if(path2Goal.tam==0 && navArea[gridPosition.first][gridPosition.second]){ // El robot no está en inflado pero no es posible obtener camino
            if(check_pos_in_grid(expl_obj.getGoalx(), expl_obj.getGoaly(), gsx, gsy) && fmm_segments.map[expl_obj.getGoalx()][expl_obj.getGoaly()]>=0 && navArea[expl_obj.getGoalx()][expl_obj.getGoaly()]){ // El goal no está en inflado
