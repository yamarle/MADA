    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        if(path2Follow.size()){
            ...
            if(path2Follow.size()){
                ...
                cmd_vel.angular.z = permissive_rotation_velocity - permissive_rotation_velocity/100;
            }
        }else{
            cmd_vel.linear.x = cmd_vel.angular.z = 0.0;
        }
        ...
        if(monitoring_type){
            if(explore == false){
                ...
                // Almacenar las variables
                    monitored_rec.push_back(exploration::explored_area(expl_obj.get_seen_map(), navAreaOrig));
                    distances_rec.push_back(mada_util::computePathDistance(totalPath));
                    times_rec.push_back(max_mission_time);
                }

                // Reiniciar variables: 
                expl_obj.set(static_map, static_map, vrange); // "exporador"
