...
int main(int argc, char** argv) {
    ...
    while(!grid.size() || currPose.position.x < -10000 || gridPosition.first < -10000){
        ...
        cout<<"EL TIEMPO: "<<ros::Time::now()<<" - "<<ros::Time::now().toSec()<<endl;
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        if(cmd_vel_laser.angular.z < -permissive_rotation_velocity || cmd_vel_laser.angular.z > permissive_rotation_velocity){
            laserPoss.clear();
        }
        ...
        if(path2Follow.size()){
            ...
            if(path2Follow.size()){
                ...
                else cmd_vel = mada_util::computeVelocitiesAcc(currPose, waypointPose, d, maxLinearVel, maxAngularVel, _cmd_vel.linear.x, _cmd_vel.angular.z, linearAcc, angularAcc);
