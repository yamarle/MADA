...
double permissive_rotation_velocity = 0.5;
...
public:
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan_in)
    {
        ...
        for(int i=0; i<cloud.points.size(); i++){
            ...
            laserPos.push_back(pose.position);
...
int main(int argc, char** argv) {
    ...
    // Variables para la proyección de puntos
    std::chrono::time_point<std::chrono::steady_clock> obst_start_timer = chrono::steady_clock::now();
    std::chrono::time_point<std::chrono::steady_clock> obst_end_timer;
    std::chrono::duration<double> obst_elapsed_time;
    float obstaclesRegTime;
    float domin; // Distancia al obstáculo dinámico más cercano
    float velT = 0.5; // Período con el que registro las velocidades
    float velTime = 1; // Tiempo que se considera para las proyectar las velocidades y poses de los obstáculos
    float maxObstRadius = 0; // Tamaño máximo de los obstáculos
    float maxObstVel = 0; // La velocidad del obstáculo más rápido

    Poss<int> all_proj_obst_poss; // Todas las posiciones de los obstáculos proyectados
    vector<geometry_msgs::Point> all_proj_obst_points;
    vector<Poss<int>> proj_obst_poss; // Todas las posiciones de los obstáculos proyectados por obstáculo

    vector<vector<geometry_msgs::Point>> obst_clusters_points; // (x,y,z): (x,y,tiempo) Todos los puntos donde se han visto los obstáculos
    vector<vector<geometry_msgs::Point>> obst_clusters_trajectories;
    vector<vector<geometry_msgs::Point>> obst_centroids_trajectories;

    std::chrono::time_point<std::chrono::steady_clock> pose_start_timer = chrono::steady_clock::now();
    std::chrono::time_point<std::chrono::steady_clock> pose_end_timer;
    std::chrono::duration<double> pose_elapsed_time;
    float poseRegTime;

    Poss<int> infl_poss; // Todas las posiciones infladas
    ...
    while(ros::ok() && explore && max_mission_time > 0){ // Hasta que explore todo el espacoio libre
        ...
        // SEGUIMIENTO DE LOS OBSTÁCULOS DINÁMICOS

        // Almacenar el tiempo en el que se registran lo obstáculos
        //obstaclesRegTime = ros::Time::now().toSec();

        obst_end_timer = chrono::steady_clock::now();
        obst_elapsed_time = obst_end_timer - obst_start_timer;
        obstaclesRegTime = obst_elapsed_time.count();

        cout<<"tiempo de registro: "<<obstaclesRegTime<<endl;
        cout<<"tiempo de registro: "<<ros::Time::now().toSec()<<endl;

        cout<<"("<<currPose.position.x<<", "<<currPose.position.y<<")"<<endl;

        cout<<visible_obstacles_pos.size()<<" obstáculos visibles"<<endl;

        // Obtener los puntos proyectados
        all_proj_obst_poss = pointProj::projection(currPose, visible_obstacles_pos, totalPath, obst_clusters_points, obst_centroids_trajectories, obst_clusters_trajectories, timeReg, domin, obstaclesRegTime, velT, velTime, navAreaOrig, laserPos, mapMsg, robotRadius, maxLinearVel, maxObstRadius, maxObstVel, all_proj_obst_points, proj_obst_poss);

        cout<<all_proj_obst_poss.x.size()<<" posiciones proyectadas"<<endl;
        cout<<obst_centroids_trajectories.size()<<" centroides"<<endl;
        cout<<obst_clusters_points.size()<<" clusters"<<endl;
        cout<<obst_clusters_trajectories.size()<<" clusters"<<endl;

        // Inflar las posiciones de las posiciones proyectadas
        //dynamic_areas::inflatePoss(all_proj_obst_poss, static_map, infl_dist, navArea, infl_poss);
        dynamic_areas::inflatePoss(all_proj_obst_poss, static_map, 2*robotRadiusGrid, navArea, infl_poss);
        cout<<"inflado10"<<endl;
        ...
        //footprintPub.publish(mada_util::pointsToPolygonStamped(mada_util::footprintNoOr(currPose, sX/2, sY/2), robot_frame));
        ...
        pose_end_timer = chrono::steady_clock::now();
        pose_elapsed_time = pose_end_timer - pose_start_timer;
        poseRegTime = pose_elapsed_time.count();
        timeReg.push_back(poseRegTime);
